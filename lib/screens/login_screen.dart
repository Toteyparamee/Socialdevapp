...
class _LoginScreenState extends State<LoginScreen>
  ...
  Future<void> _checkSchoolAndProceed() async {
    final auth = context.read<AuthService>();
    if ((auth.schoolId ?? 0) == 0) {
      // โหลดรายชื่อโรงเรียน
      ...
      try {
        ...
        if (res.statusCode == 200) {
          schools = List<Map<String, dynamic>>.from(jsonDecode(res.body));
        }
      } catch (_) {}
      ...
      await AwesomeDialog(
          ...
          builder: (ctx, setS) => Padding(
            ...
            child: Column(
              children: [
                ...
                const Text(
                  'กรุณาเลือกโรงเรียนของคุณเพื่อใช้งานระบบ',
        ...
        btnCancelText: 'ข้ามไปก่อน',
        btnCancelOnPress: () {},
        btnOkText: 'ยืนยัน',
        btnOkOnPress: () async {
          if (selectedId == null) return;
          try {
            await auth.updateSchool(selectedId!, selectedName);
          } catch (_) {}
        },
      ).show();
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (mounted) await _checkSchoolAndProceed();
