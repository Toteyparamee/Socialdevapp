...
class ProblemService extends ChangeNotifier {
  ...
  Future<void> fetchProblems({String? category, String? status}) async {
    ...
    try {
      ...
      debugPrint('>>> TOKEN: ${_auth.token}');
      debugPrint('>>> fetchProblems calling $uri');
      final response = await http.get(uri, headers: _auth.authHeaders).timeout(_timeout);
      debugPrint('>>> fetchProblems status=${response.statusCode} body=${response.body.substring(0, response.body.length.clamp(0, 300))}');
      if (response.statusCode == 200) {
        final list = jsonDecode(response.body) as List;
        _problems = list.map((j) => ProblemReport.fromJson(j)).toList();
        debugPrint('>>> loaded ${_problems.length} problems');
      } else {
  ...
  Future<ProblemReport?> getProblem(String id) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/$id'), headers: _auth.authHeaders).timeout(_timeout);
      if (response.statusCode == 200) {
